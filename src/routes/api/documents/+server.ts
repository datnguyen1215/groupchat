...
export const POST: RequestHandler = async ({ request }) => {
	...
	const problems = [];
	if (!(await threadExists(threadId!))) problems.push({ field: 'threadId', message: 'no such thread' });
	if (!(await agentExists(authorId!))) problems.push({ field: 'authorId', message: 'no such agent' });
	if (problems.length) fail(422, 'Validation failed', problems);

	const id = await uniqueId(documents, name!);
	await db.insert(documents).values({ id, name: name!, threadId: threadId!, authorId: authorId!, body: text });
