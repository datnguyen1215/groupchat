		...
		{#each groups as group, g (group.label)}
			...
			{#each group.steps as step, i (step.id)}
				<div class="relative mb-[2px] {step.child ? 'pl-[52px]' : 'pl-[26px]'}">
					...
					<div class="flex items-center gap-[10px] rounded-[7px] px-[9px] py-[9px] hover:bg-panel-2">
						<span class="font-mono text-[11.5px] tracking-[-0.02em]">{step.name}</span>
						{#if step.badge}
							<span class="rounded bg-accent/[0.13] px-[5px] py-px text-[9px] font-bold tracking-[0.04em] text-accent uppercase">
								{step.badge}
							</span>
						{/if}
