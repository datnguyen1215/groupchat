...
<div class="min-h-0 flex-1 overflow-y-auto">
  <div class="mx-auto max-w-[1000px] px-[30px] pt-[34px] pb-[60px]">
    ...
    >
      {#snippet actions()}
        <button
          class="rounded-lg border border-line px-[13px] py-[6px] text-[12.5px] font-medium text-ink-2 hover:bg-panel-2 hover:text-ink"
        >
          Import
        </button>
        <button
          class="rounded-lg border border-ink bg-ink px-[13px] py-[6px] text-[12.5px] font-semibold text-bg hover:bg-white"
        >
          New skill
        </button>
      {/snippet}
      ...
      {#each shown as skill (skill.id)}
        ...
        >
          <span class="mb-2 flex items-center gap-[9px]">
            <span
              class="grid h-6 w-6 flex-none place-items-center rounded-[7px] bg-violet/[0.14] text-[11px] text-violet"
              >◈</span
            >
            <span class="font-mono text-[12.5px] font-semibold tracking-[-0.02em]"
              >{skill.name}</span
            >
            <span
              class="ml-auto rounded-full bg-line-2 px-[6px] py-[1.5px] font-mono text-[10px] text-ink-3"
            >
              {skill.version}
            </span>
          </span>
          ...
          <span
            class="flex items-center gap-[6px] border-t border-line-2 pt-[11px] text-[11px] text-ink-3"
          >
            <Avatar
              initials={skill.authorInitials}
              color={skill.authorColor}
              size="xs"
              shape="circle"
            />
