        ...
        <thead>
          <tr>
            {#each ['Name', 'Thread', 'Author', 'Updated', ''] as heading (heading)}
              <th
                class="border-b border-line px-3 pb-[9px] text-left text-[10.5px] font-semibold tracking-[0.06em] text-ink-3 uppercase"
              >
                {heading}
              </th>
            {/each}
        ...
        <tbody>
          {#each shown as doc (doc.id)}
            <tr class="cursor-pointer hover:bg-panel" onclick={() => overlay.openDoc(doc.id)}>
              ...
              <td class="border-b border-line-2 px-3 py-[11px] text-[12px] text-ink-3"
                >{doc.updated}</td
              >
              <td class="border-b border-line-2 px-3 py-[11px] text-[12px] text-ink-3"
                >{doc.version}</td
