...
<div class="text-[14px]/[1.7] text-[#d6d6d4]">
  {#each blocks as block, i (i)}
    ...
    {:else if block.type === 'ordered'}
      <ol class="mb-[14px] list-decimal pl-[22px]">
        {#each block.items as item, j (j)}
          <li class="mb-[6px]"><Inline text={item} /></li>
        {/each}
      </ol>
    ...
    {:else if block.type === 'code'}
      <pre
        class="mb-[15px] overflow-x-auto rounded-[9px] border border-line bg-[#0b0b0d] px-4 py-[14px]"><code
          class="font-mono text-[12px]/[1.65] text-[#c9c9c6]">{block.text}</code
        ></pre>
          ...
          <thead>
            <tr>
              {#each block.head as cell, j (j)}
                <th
                  class="border border-line bg-line-2 px-3 py-[7px] text-left text-[12px] font-semibold text-ink"
                >
                  <Inline text={cell} />
                </th>
              {/each}
